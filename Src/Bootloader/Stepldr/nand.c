...
extern void _Read_512Byte(BYTE *bufPt, UINT32 nNFDATAAddr);		// in "nand_opt.s"
...
void NAND_Init(void)
{
	...
	//
	rMEM_SYS_CFG = (rMEM_SYS_CFG & ~(0x1000)); // 8-bit data width
	rMEM_SYS_CFG = (rMEM_SYS_CFG & ~(0x3F)) | (0x00); // Xm0CSn[2] = NFCON CS0
...
BOOL NAND_ReadPage(UINT32 block, UINT32 sector, UINT8 *buffer)
{
	...
	for (nSectorLoop = 0; nSectorLoop < (g_bLargeBlock==TRUE?4:1); nSectorLoop++)
	{
		...
		_Read_512Byte(bufPt+nSectorLoop*512, NFDATA);	// Read 512 bytes.
		...
		if (g_bLargeBlock == TRUE)
		{
			...
			if (((rddata1&0xff) != 0xff) && ((rddata2 & 0xff) != 0xfc))
			{
				...
				Uart_SendBYTE(block,1);
				Uart_SendString("sector \r\n");
				Uart_SendBYTE(sector,1);
				Uart_SendString("rddata1 \r\n");
				Uart_SendBYTE(rddata1,1);
				Uart_SendString("rddata2 \r\n");
				Uart_SendBYTE(rddata2,1);
		...
		if (g_bLargeBlock == TRUE)
		{
            // ColAddr = ( 2048 + 1(Bad) + 4(Rsv) + 1(OEMRsv) + 2(Rsv2) ) + MECCx
            // ColAddr = ( 2048 + 1(Bad) + 4(Rsv) + 1(OEMRsv) + 2(Rsv2) ) + 2(Copy1Rsv2) + 2(Copy2Rsv2) + 4(ECC of first 6 bytes) + MECCx
			ColAddr = 2048 + 16 + nSectorLoop*4;
