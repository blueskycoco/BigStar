...
void ChooseImageFromSD()
{
	...
	EdbgOutputDebugString ( "0) SuperIPL.nb0\r\n" );
	EdbgOutputDebugString ( "1) EBOOT.BIN\r\n" );
	EdbgOutputDebugString ( "2) Flash.bin\r\n" );
    ...
    if (IsCardInserted())
    {
	    ...
	    {
	    case '0':
			Parsing("superipl.nb0", NB0IMAGE, (BYTE *)DEF_DOWNLOAD_ADDR);
	        break;
	    ...
	    case '1':
	        ...
	        break; 
        case '4':
			Parsing("Eboot-b.bin", BINIMAGE, (BYTE *)DEF_DOWNLOAD_ADDR);
	        break; 
	    case '2':
    		Parsing("Flash.bin", BINIMAGE, (BYTE *)DEF_DOWNLOAD_ADDR);
	        break;
	    case '3':
...
void FlashReadFromSD()
{
    ...
    g_pDownPt = (UINT8 *)FLASH_DOWNLOAD_ADDR;
    readPtIndex = (UINT32)FLASH_DOWNLOAD_ADDR;
    
    memset((void*)FLASH_DOWNLOAD_ADDR, 0xff, FILE_CACHE_LENGTH);

    if (IsCardInserted())
    {
		//FATInit();
		
        Parsing("Flash.bin", BINIMAGE, (BYTE *)FLASH_DOWNLOAD_ADDR);
...
void LogoReadFromSD()
{
    ...
    if (IsCardInserted())
    {
		...
		Parsing("Logo.bin", BINIMAGE, (BYTE *)DEF_DOWNLOAD_ADDR);
    	
    		EdbgOutputDebugString ( "g_pDownPt = 0x%x \r\n", g_pDownPt);
		WriteRawImageToBootMedia((BYTE *)DEF_DOWNLOAD_ADDR, 0xaf000, 0x10);
	}
}
