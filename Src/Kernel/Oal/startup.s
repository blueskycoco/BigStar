...
;-----------------------------------------------------
;	6.1 Check EINT0PEND Bypass Sleep
;-----------------------------------------------------
...
;-----------------------------------------------------
        ...
        ldr r3, =vSLPEN
        mov r4, #0x2                 ; clear sleep_en

        ldr r5, =vPWR_CFG
        ldr r6, [r5]
        bic r6, r6, #0x61			; Clear STANDBYWFI
        orr r6, r6, #0x60			; Enter SLEEP mode

        ldr r7, =vSLEEP_CFG
        ldr r8, [r7]
        bic r8, r8, #0x61			; Disable OSC_EN (Disable X-tal Osc Pad in Sleep mode)

        ; check bypass sleep
        tst r1,r2
        bne WakeUp_Address           ; Branch Bypass Sleep

        ; Go to sleep as soon as possible for avoiding to access a memory
        str r4, [r3]
        str r6, [r5]
        str r8, [r7]
