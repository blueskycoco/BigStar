...
static void InitializeGPIO()
{
	...
	OALMSG(TRUE, (L"[OAL] ++GPS Power On Enable\r\n"));
	//GPN7 GPS power enable high level valid
	pGPIOReg->GPNCON = (pGPIOReg->GPNCON & ~(0x03<<14))|(1<<14);
	pGPIOReg->GPNPUD = (pGPIOReg->GPNPUD & ~(0x3<<14)) | (0<<14);
	pGPIOReg->GPNDAT |= (1<<7);

	OALMSG(TRUE, (L"[OAL] ++3G Power On Queue\r\n"));

	//GPK10  3G PowerON/OFF
	//Init for high level
	pGPIOReg->GPKCON1 = (pGPIOReg->GPKCON1 & ~(0x0F<<8))|(1<<8);
	pGPIOReg->GPKPUD = (pGPIOReg->GPKPUD & ~(0x3<<20)) | (0<<20);
	pGPIOReg->GPKDAT |= (1<<10);

	//GPK14  MD_PWR_EN Power Enable
	pGPIOReg->GPKCON1 = (pGPIOReg->GPKCON1 & ~(0x0F<<24))|(1<<24);
	pGPIOReg->GPKPUD = (pGPIOReg->GPKPUD & ~(0x3<<28)) | (0<<28);
