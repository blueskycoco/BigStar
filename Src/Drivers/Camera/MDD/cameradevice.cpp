    ...
    )
{
    ...
    if ( NULL == m_hStream )
    {
        RETAILMSG( dbg, ( _T("CAM_Init: ActivateDevice on Pin failed\r\n") ) );
        goto FailAndExit;
    }
    ...
    if( false == GetPDDPinInfo() )
    {
        delete m_pStrmInstances;
        m_pStrmInstances = NULL;
        RETAILMSG(dbg, (_T("CAM_IOControl(%08x): Failed to retrieve sub-device information\r\n"), this)) ;
        goto FailAndExit;
    }
...
FailAndExit:
	RETAILMSG(dbg, (_T("CCameraDevice::Initialize failed!!\r\n"), this)) ;
	return false;
}
...
CCameraDevice::GetPDDPinInfo()
{
    ...
    if( NULL == m_pStrmInstances )
    {
    	RETAILMSG(dbg,(TEXT("GetPDDPinInfo erro1\r\n")));
        return false;
    }
    ...
    for( UINT i=0; i < m_AdapterInfo.ulCTypes; i++ )
    {
        ...
        {
        	RETAILMSG(dbg,(TEXT("GetPDDPinInfo erro2\r\n")));
            return false ;
        }
        ...
        {
        	RETAILMSG(dbg,(TEXT("GetPDDPinInfo erro3\r\n")));
            return false;
        }
        ...
        {
        	RETAILMSG(dbg,(TEXT("GetPDDPinInfo erro4\r\n")));
            return false;
        }
    }
...
{
    RETAILMSG( dbg, ( _T("CAM_Close: Unbind application from camera device\r\n") ) );
...
bool
CCameraDevice::CamOpenInit(DWORD dwDeviceIndex)
{
	m_PDDFuncTbl.PDD_OpenInit(m_PDDContext,dwDeviceIndex);
	return true;
}

bool
CCameraDevice::CamClosed()
{
	m_PDDFuncTbl.PDD_Closed(m_PDDContext);
	return true;
}
    ...
    )
{
    for ( ULONG ulCount = 0 ; ulCount < m_pStrmInstances[ulPinId].pVideoFormat->ulAvailFormats ; ulCount++ )
    {
		...
		RETAILMSG(dbg,(TEXT("AdapterCompareFormat  width  %d  %d\r\n"),pCsDataRangeVideoToCompare->VideoInfoHeader.bmiHeader.biWidth,pCsDataRangeVideo->VideoInfoHeader.bmiHeader.biWidth));
    ...
    )
{
    ...
    {
        RETAILMSG( dbg, (_T("IOControl(%08x): buffer is not large enough\r\n"), this ) );
        *dwError = ERROR_INSUFFICIENT_BUFFER;
    ...
    {
        RETAILMSG( dbg, (_T("IOControl(%08x): MapCallerPtr failed for buffer.\r\n"), this ) );
...
{
    //RETAILMSG( ZONE_IOCTL, ( _T("CAM_IOControl(%08x): HandlePinRequests\r\n"), this ) );
    ...
    *pdwBytesTransferred = 0;
    ...
    switch( csProp.Id )
    {
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): CSPROPERTY_PIN_CTYPES\r\n"), this ) );
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): CSPROPERTY_PIN_CINSTANCES\r\n"), this ) );
            ...
            {
                RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Invalid PinId\r\n"), this ) );
                break;
            }
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): CSPROPERTY_PIN_DATARANGES\r\n"), this ) );
            ...
            *pdwBytesTransferred = sizeof(CSMULTIPLE_ITEM) + ( m_pStrmInstances[lPinId].pVideoFormat->ulAvailFormats * sizeof (CS_DATARANGE_VIDEO) );
			...
			RETAILMSG(dbg,(TEXT("CSPROPERTY_PIN_DATARANGES  m_pStrmInstances[%d].pVideoFormat->ulAvailFormats=%d\r\n"),lPinId,m_pStrmInstances[lPinId].pVideoFormat->ulAvailFormats));	
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): CSPROPERTY_PIN_DATAINTERSECTION\r\n"), this ) );
            if ( NULL == ( pCsPin = reinterpret_cast<PCSP_PIN>( ValidateBuffer( pInBuf, InBufLen, sizeof (CSP_PIN), &dwError ) ) ) )
            {
                break;
            }
...

            {
                for ( int iCount = 0 ; iCount < (int)pCsMultipleItem->Count ; iCount++ )
                {
                    ...
                    {
                        // We found our format
                        RETAILMSG(dbg,(TEXT("CSPROPERTY_PIN_DATAINTERSECTION  We found our format %d for pin %d\r\n"),iCount,lPinId));
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): CSPROPERTY_PIN_CATEGORY\r\n"), this ) );
            ...
            *pdwBytesTransferred = sizeof( GUID );
            ...
            {
				RETAILMSG( dbg, (_T("CAM_IOControl CSPROPERTY_PIN_CATEGORY for pin %d\r\n"),lPinId ) );
                dwError = ERROR_SUCCESS;
            }
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): Property CSPROPERTY_PIN_NAME\r\n"), this ) );
            ...
            *pdwBytesTransferred = ( wcslen(g_wszPinNames[lPinId] ) + 1 ) * sizeof( g_wszPinNames[0][0] );
            ...
            {
				RETAILMSG( dbg, (_T("CAM_IOControl  CSPROPERTY_PIN_NAME for pin %d\r\n"), lPinId) );
                dwError = ERROR_SUCCESS;
            }
        ...
        {
            //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): Property CSPROPERTY_PIN_DEVICENAME\r\n"), this ) );
            ...
            *pdwBytesTransferred = ( wcslen( g_wszPinDeviceNames[lPinId] ) + 1 ) * sizeof( g_wszPinDeviceNames[0][0] ) ;
            ...
            {
				RETAILMSG( dbg, (_T("CAM_IOControl  CSPROPERTY_PIN_DEVICENAME for pin %d\r\n"), lPinId) );
                dwError = ERROR_SUCCESS;
            }
        ...
        default:
        {
            RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Invalid Property\r\n"), this ) );
    ...
    )
{
    ...
    //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): HandleVidProcAmpRequests\r\n"), this ) );
    ...
    *pdwBytesTransferred = 0;
    ...
    default :
        RETAILMSG(dbg, (_T("CAM_IOControl(%08x): Invalid Request\r\n"), this));
        return dwError;
    }
    ...
    )
{
    ...
    //RETAILMSG( ZONE_IOCTL, ( _T("CAM_IOControl(%08x): HandleCamControlRequests\r\n"), this ) );
    DWORD dwError = ERROR_INVALID_PARAMETER;
    LONG  lValue  = 0;
    LONG  lFlags  = 0;
    PSENSOR_PROPERTY pSensorProp = NULL;
    ...
    *pdwBytesTransferred = 0;
	...
	if(m_bLtkMode)
	{
	    if ( csProp.Id < CSPROPERTY_CAMERACONTROL_PAN || csProp.Id > CSPROPERTY_CAMERACONTROL_WORKMODE )
	    {
	        RETAILMSG(dbg, (_T("CAM_IOControl(%08x): Invalid Property\r\n"), this));
	        return dwError;
	    }
...
{
    RETAILMSG( dbg, (_T("CAM_IOControl(%08x): HandleCompressionRequests\r\n"), this ) );
...
{
    // Note: This whole function is wrapped in a __try/__except block
    RETAILMSG( dbg, (_T("CAM_IOControl(%08x): HandleVideoControlRequests\r\n"), this ) );
    ...
    *pdwBytesTransferred = 0;
    ...
    case CSPROPERTY_VIDEOCONTROL_CAPS:
        ...
        case CSPROPERTY_TYPE_GET:
            *pdwBytesTransferred = sizeof(CSPROPERTY_VIDEOCONTROL_CAPS_S);
			...
			RETAILMSG(dbg,(TEXT("CSPROPERTY_VIDEOCONTROL_CAPS  GetPDDPinInfo StreamIndex=%d\r\n"),lIndex));	
        ...
        case CSPROPERTY_TYPE_SET:
            ...
            {
                RETAILMSG( dbg, ( _T("CAM_IOControl(%08x): Invalid flag specified as Video Control Caps.\r\n"), this ) );
                break;
            }

            if( false == IsValidPin( lIndex ))
            {
                RETAILMSG( dbg, ( _T("CAM_IOControl(%08x): Invalid pin index.\r\n"), this ) );
                break;
            }
    ...
    case CSPROPERTY_VIDEOCONTROL_MODE:
        ...
        case CSPROPERTY_TYPE_SET:
            ...
            {
                RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Invalid flag specified as Video Control Mode.\r\n"), this ) );
                break;
            }
            ...
            {
                RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Invalid pin for Trigger.\r\n"), this ) );
            }
...
{
    // Note: This whole function is wrapped in a __try/__except block
    RETAILMSG( dbg, (_T("CAM_IOControl(%08x): HandleDroppedFramesRequests\r\n"), this ) );
    ...
    )
{
    ...
    RETAILMSG( dbg, ( _T("CAM_IOControl(%08x): HandlePowerRequests\r\n"), this ) );
    ...
    case IOCTL_POWER_CAPABILITIES:
        ...
        RETAILMSG(dbg, (TEXT("CAM: IOCTL_POWER_CAPABILITIES\r\n")));
        if ( pOutBuf && OutBufLen >= sizeof(POWER_CAPABILITIES) )
        {
             memcpy(pOutBuf, &m_AdapterInfo.PowerCaps, sizeof(POWER_CAPABILITIES));
        	 RETAILMSG(dbg, (TEXT("CAM: IOCTL_POWER_CAPABILITIES %x\r\n"),m_AdapterInfo.PowerCaps.DeviceDx));
    ...
    case IOCTL_POWER_SET:
        ...
        RETAILMSG(dbg, (TEXT("CAM: IOCTL_POWER_SET\r\n")));
        if ( pOutBuf && OutBufLen >= sizeof(CEDEVICE_POWER_STATE) )
        {
            ...
            default:
                RETAILMSG(dbg, (TEXT("CAM: Unrecognized Valid Power state. Powering Up\r\n")));
    ...
    case IOCTL_POWER_GET:
        ...
        RETAILMSG(dbg, (TEXT("CAM: IOCTL_POWER_GET\r\n")));
        if ( pOutBuf && OutBufLen >= sizeof(CEDEVICE_POWER_STATE) )
        {
...
{
    // Note: This whole function is wrapped in a __try/__except block
    //RETAILMSG( ZONE_IOCTL, (_T("CAM_IOControl(%08x): AdapterHandleCustomRequests\r\n"), this ) );
    ...
    )
{
    ...
    if ( OutBufLen >= sizeof(CSPROPERTY_DESCRIPTION) )
    {
        ...
        {
            RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Not all available data could be written\r\n"), this ) );
            *pdwError = ERROR_MORE_DATA;
        }
    }
    else if ( OutBufLen >= sizeof(ULONG) )
    {
        ...
        RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Returning AccessFlags\r\n"), this ) );
    ...
    {
        RETAILMSG( dbg, (_T("CAM_IOControl(%08x): Output buffer is not large enough\r\n"), this ) );
