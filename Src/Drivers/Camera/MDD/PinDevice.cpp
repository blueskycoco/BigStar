...
DWORD CPinDevice :: StreamInstantiate( PCSPROPERTY_STREAMEX_S pCsPropStreamEx, PUCHAR pOutBuf, DWORD  OutBufLen, PDWORD pdwBytesTransferred )
{
    ...
    if ( -1 != m_ulPinId )
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): Pin %d is already instantiated.\r\n"), this, m_ulPinId )) ;
        return dwError ;
    }

    if( NULL == m_pCamAdapter )
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): Initialization incomplete.\r\n"), this, m_ulPinId )) ;
        return dwError;
    }

    if ( false == m_pCamAdapter->IsValidPin( pCsPropStreamEx->CsPin.PinId ) )
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): Invalid Pin Id\r\n"), this)) ;
        return dwError ;
    }
    ...
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): Failed to retrieve sub-device information\r\n"), this)) ;
        return dwError ;
    }
    ...
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): No Pin Format provided for pin\r\n"), this)) ;
        return dwError ;
    }
    ...
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): NULL Handle provided for msgqueue\r\n"), this)) ;
        return dwError ;
    }    
    ...
    {
        RETAILMSG(dbg, (_T("PIN_IOControl(%08x): Failed to open MsgQueue\r\n"), this)) ;
        return dwError ;
    }
...
void CPinDevice :: SetState( CSSTATE CsState, CSSTATE *CsPrevState )
{
    ...
    if(m_CsState != CsState)
    {
        ...
        RETAILMSG(dbg, (_T("Pin: %d Setting State to 0x%X\r\n"), m_ulPinId, CsState)) ;
    }
...
BOOL CPinDevice::InitMsgQueueDescriptor (PCS_MSGQUEUE_BUFFER pCsMsgQBuff, PCS_STREAM_DESCRIPTOR pCsStreamDesc, PVOID pMappedData, PVOID pUnmappedData, BOOL bBufferFill)
{
	...
	static DWORD dwTimeTem=0,dwPreTime=0;
    //    RETAILMSG(1,(TEXT("InitMsgQueueDescriptor\n")));
    if(( pCsStreamHeader == NULL ) || ( pCsFrameInfo == NULL ))
    {
        RETAILMSG(dbg, (_T("InitMsgQueueDescriptor(%08x): Invalid Stream Descriptor\r\n"), this));
        return false;
    }
    ...
    //RETAILMSG(ZONE_FUNCTION, (_T("InitMsgQueueDescriptor: LastPT = %d, elapsed = %d\r\n"), m_msLastPT, m_msLastPT - prevPT));
	...
	if(m_ulPictureNumber%100==0)
	{
		dwTimeTem = GetTickCount();
		RETAILMSG(1 ,(L"## fps = %d ##, droped %d frames\r\n",100000/(dwTimeTem-dwPreTime),m_ulFramesDropped));
		dwPreTime = dwTimeTem;
	}

    //RETAILMSG(ZONE_FUNCTION, (_T("InitMsgQueueDescriptor(%08x): Frame buf queued: %d (dropped %d), start %d, time %d\r\n"), 
    //    this,
...
void  CPinDevice::FlushBufferQueue()
{
    ...
    while (( true == RemoveBufferFromList( &pCsStreamDesc, &pMappedData, &pUnmappedData )) && ( NULL != pCsStreamDesc ) && ( m_hMsgQ != NULL ))
    {
        ...
        {
            RETAILMSG(dbg, (_T("PIN_Function(%08x): WriteMsgQueue returned false\r\n"), this));
        }
    }    
...
{    
    ...
    if ( NULL == m_hMsgQ )
    {
        RETAILMSG(dbg, (_T("PIN_Function(%08x): MsgQueue is not opened\r\n"), this)) ;         
        bOK = FALSE;
        goto exit;
    }
        ...
        )
{
    ...
    case CSPROPERTY_CONNECTION_DATAFORMAT:
        ...
        __except( EXCEPTION_EXECUTE_HANDLER )
        {
            ...
            RETAILMSG(dbg, (_T("PIN_IOControl(%08x): CSPROPERTY_CONNECTION_DATAFORMAT: Exception occured\r\n"), this)) ;
        }
    ...
    case CSPROPERTY_CONNECTION_ALLOCATORFRAMING:
        ...
        default :
            RETAILMSG(dbg, (_T("PIN_IOControl(%08x): CSPROPERTY_CONNECTION_ALLOCATORFRAMING Invalid Request\r\n"), this)) ;
        }
    ...
    )
{
    ...
    {
        case CS_ALLOCATE:
            dwError = AllocateBuffer( pOutBuf, OutBufLen, pdwBytesTransferred );
            break;
    ...
    )
{
    ...
    case CSPROPERTY_TYPE_SET:
        ...
        case CSSTATE_STOP:
            ...
            {
                RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): State to set = CSSTATE_STOP but we are already Stopped.\r\n"), this ) );
                dwError = ERROR_SUCCESS;
                break;
            }
        ...
        case CSSTATE_PAUSE:
            ...
            {
                RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): State to set = CSSTATE_PAUSE but we are already Paused.\r\n"), this ) );
                dwError = ERROR_SUCCESS;
                break;
            }
        ...
        case CSSTATE_RUN:
            ...
            if ( CSSTATE_STOP == m_CsState ) 
            {
                RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): CSSTATE_STOP to CSSTATE_RUN is not a supported transition .\r\n"), this ) );
        ...
        default :
            RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): Incorrect State\r\n"), this ) );
            dwError = ERROR_INVALID_PARAMETER;
        }
        LeaveCriticalSection(&m_csStreamIO);
        break;
...
DWORD 
CPinDevice::AllocateBuffer( LPVOID pOutBuf, DWORD  OutBufLen, DWORD *pdwBytesTransferred )
{
    DWORD                   dwError = ERROR_INVALID_PARAMETER;
    DWORD                   dwResult = -1;
    PCS_STREAM_DESCRIPTOR   pCsDescriptor = (PCS_STREAM_DESCRIPTOR) pOutBuf;
...
CPinDevice::DeallocateBuffer(  LPVOID pOutBuf  )
{
    ...
    if( m_dwMemoryModel == CSPROPERTY_BUFFER_DRIVER )
    {
        ...
        dwError = m_pCamAdapter->PDDDeAllocatePinBuffer( m_ulPinId, m_pStreamDescriptorList[ dwIndex ].csStreamDescriptorShadow.CsStreamHeader.Data );
        m_pStreamDescriptorList[ dwIndex ].csStreamDescriptorShadow.CsStreamHeader.Data = NULL;
        pCsDescriptor->CsStreamHeader.Data = NULL;
    }
    else
    {
        dwError = m_pCamAdapter->PDDUnRegisterClientBuffer( m_ulPinId, m_pStreamDescriptorList[ dwIndex ].csStreamDescriptorShadow.CsStreamHeader.Data );
...
{
    DWORD  dwHandle;
    DWORD dwProcessId = (DWORD)GetCallerProcess();
...
CPinDevice::EnqueueDescriptor(  LPVOID pOutBuf )
{
	...
	// When camera suspend/resume, Application cannot regist buffer by this comparison statement.
	// So comment out 07.07.24 JJG
	/*		
    if( m_CsState == CSSTATE_STOP )
    {
    ...
    if( NULL == pCsDescriptor )
    {
    	RETAILMSG( dbg, ( _T("EnqueueDescriptor NULL == pCsDescriptor\r\n"), this ) );
        return dwError;
    }
    ...
    if( dwIndex == -1 )
    {
    	RETAILMSG( dbg, ( _T("EnqueueDescriptor dwIndex == -1\r\n"), this ) );
        goto Cleanup;
    }
    ...
    {
        RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): The buffer has not be prepared. Call CS_ALLOCATE first.\r\n"), this ) );
        goto Cleanup;
    }

    if( m_pStreamDescriptorList[ dwIndex ].pCsStreamDescriptorExternal != NULL )
    {
        RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): This buffer has already be enqueued.\r\n"), this ) );
        goto Cleanup;
    }
    ...
    {
        RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): MapCallerPtr failed for incoming pointer \r\n"), this ) );
        goto Cleanup;
    }
    ...
    )
{
    ...
    //RETAILMSG( ZONE_IOCTL, ( _T("PIN_IOControl(%08x): RemoveBufferFromList\r\n"), this ) );
    if(( ppCsStreamDesc == NULL ) || ( ppMappedData == NULL ) || ( ppUnmappedData == NULL ))
    {
        RETAILMSG( dbg, ( _T("PIN_IOControl(%08x): RemoveBufferFromList - Null pointer has been passed in.\r\n"), this ) );
        return RetVal;
    }
    ...
    while(( dwCounter < m_dwBufferCount ) && ( *ppCsStreamDesc == NULL ))
    {
        ...
        if( m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal != NULL )
        {
            ...
            *ppUnmappedData = m_pStreamDescriptorList[ dwCounter ].pCsStreamDescriptorExternal->CsStreamHeader.Data;
            ...
            {
		        RETAILMSG( dbg, ( _T("Exception\r\n")) );            
                RETAILMSG(dbg, (_T("PIN_IOControl: RemoveBufferFromList - Access Violation.\r\n"))) ;                
                RetVal = false;
            }         
        }
