...
extern "C" {
    ...
    // PDD_Init is called when the Cam_Open is called. Module relative init is done here.
	DWORD PDD_OpenInit( LPVOID PDDContext, DWORD dwDeviceIndex );

    // PDD_Init is called when the Cam_Close is called. Lenovo extend relative things is done here.
	DWORD PDD_Closed( LPVOID PDDContext );
