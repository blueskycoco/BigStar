...
public:
    ...
    DWORD PDDPropsInit(DWORD dwDeviceIndex);
	DWORD PDDVideoFormatsInit(DWORD dwDeviceIndex);
    DWORD PDDClosed();
...
private:
   ...
   CRITICAL_SECTION m_csPddDevice; 	
	...
	FrameData 		*m_pMappedStruct;
	HANDLE			m_hPFrameDataOKEvent;
	HANDLE			m_hSFrameDataOKEvent;
	BOOL			m_bNewPFramedata;
	BOOL			m_bNewSFramedata;
    HANDLE      	m_hPreStill;
	BOOL			m_bDShowPreviewDataEnable;
	BOOL			m_bGetStillDataDirectEnable;
	BOOL			m_bContinousStillEnable;
	DWORD m_JpgSize;
