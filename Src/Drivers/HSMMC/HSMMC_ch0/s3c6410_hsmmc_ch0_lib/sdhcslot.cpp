	...
	if (!m_fCardPresent) {
		...
		if ((pRequest->SystemFlags & SD_FAST_PATH_AVAILABLE)!=0) { // Fastpath
			...
			if( status == SD_API_STATUS_PENDING ) { // Polling for completion.
				...
				while (m_pCurrentRequest && ((IsCardPresent() & TRUE)!=0) ) {
#else
...
#ifndef _FB_ON_
	RETAILMSG(1,(TEXT("[HSMMC0] Turn OFF the F/B delay control.\r\n")));
	dwControlValue &= ~(0x3<<14); // Turn off the feedback clock delay
#else
	RETAILMSG(1,(TEXT("[HSMMC0] Turn ON the F/B delay control.\r\n")));
