    ...
    )
{
	...
	if(dwTimeOutCount > 999)
		RETAILMSG(dbg,(_T("[UFNPDD] TX & RX FIFO Flush TimeOut Error\r\n"))); 
}
    ...
    )
{
	...
	if (dwTimeOutCount > 999)
		RETAILMSG(dbg,(_T("[UFNPDD] SoftCoreReset TimeOut Error\r\n")));
}
	...
	)
{
	for(DWORD i = 0 ; i < dwWaitCount ; i++)
	{
		...
		if (i >= (dwWaitCount-1))
		{
			RETAILMSG(dbg,(_T("[UFNPDD] TX FIFO Not Empty Error!!! Wait Coutnt %d - StartPreparedInTrasfer\r\n"),i));
			return FALSE;
		}
	}
	return TRUE;
	...
	)
{
			...
			&& (pContext->UDR.wIndex & 0xFF) == 0) 
	{
		...
		RETAILMSG(dbg, (_T("[UFNPDD] USB_FEATURE_TEST_MODE\r\n")));
		// Set TEST MODE

		RETAILMSG(dbg, (_T("[UFNPDD] TEST_FORCE_ENABLE\r\n")));
		...
		switch( wTestMode)
		{
			case USB_TEST_J:
				...
				RETAILMSG(dbg, (_T("[UFNPDD] USB_TEST_J\r\n")));
				WriteReg(DIEPTSIZ0, 1<<PACKET_COUTNT_IDX | 0);
				WriteReg(DIEPCTL0, EP_ENABLE | CLEAR_NAK | 3<<0);	//ep0 enable, clear nak, 8byte
				dwDCTL = ReadReg( DCTL);
			...
			case USB_TEST_K:
				...
				RETAILMSG(dbg, (_T("[UFNPDD] USB_TEST_K\r\n")));
				WriteReg(DIEPTSIZ0, 1<<PACKET_COUTNT_IDX | 0);
				WriteReg(DIEPCTL0, EP_ENABLE | CLEAR_NAK | 3<<0);	//ep0 enable, clear nak, 8byte
				dwDCTL = ReadReg( DCTL);
			...
			case USB_TEST_SE0_NAK:
				...
				RETAILMSG(dbg, (_T("[UFNPDD] USB_TEST_SE0_NAK\r\n")));
				WriteReg(DIEPTSIZ0, 1<<PACKET_COUTNT_IDX | 0);
				WriteReg(DIEPCTL0, EP_ENABLE | CLEAR_NAK | 3<<0);	//ep0 enable, clear nak, 8byte
				dwDCTL = ReadReg( DCTL);
			...
			{
				RETAILMSG(dbg, (_T("[UFNPDD] USB_TEST_PACKET\r\n")));
				DWORD cbWritten = 0;
				WORD WriteData = 0;
				...
				RETAILMSG(dbg,(_T("[UFNPDD] DCTL: 0x%x\r\n"),dwDCTL));				
				WriteReg(DCTL, dwDCTL);
			}
			break;
	...
	)
{
	...
	{	
		if(pContext->dwPipelinedXfered[dwEndpoint][pContext->dwPipelinedStrIdx] == 0)
		{
			...
			if (dwTimeOutCount>999)
				RETAILMSG(dbg,(TEXT("Clear NAK TimeOut Error %d [HandleOutEvent]\r\n"),dwTimeOutCount));		}
		else
		{
			RETAILMSG(dbg,(TEXT("RING BUFFER STATE : FULL\r\n")));
			DisableEndpointInterrupt(pContext, peps->dwEndpointNumber, peps->dwDirectionAssigned);
	...
	)
{
    ...
    if (pContext->attachedState == UFN_DETACH)
    {
        if(dwGOTGCTL & (B_SESSION_VALID|A_SESSION_VALID))
        {
			...
			// While running above Sleep() function, if system enters to the sleep mode, 
			// System will be lock-up. So below return code should be implemented.
	        //if (pContext->fCableAttached == FALSE)
		    //{
		    //    return;
            //}
			...
			RETAILMSG(dbg,(_T("[UFNPDD] OTG Cable Attached\r\n")));
        }
        else
        {
            pContext->pfnNotify(pContext->pvMddContext, UFN_MSG_BUS_EVENTS, UFN_DETACH);
            pContext->attachedState = UFN_DETACH;                
			RETAILMSG(dbg,(_T("[UFNPDD] RESET Exeption \r\n")));
        }
    }
    else
    {
        ...
        {		
            ...
            RETAILMSG(dbg,(_T("[UFNPDD] RESET Again \r\n")));
        }
    }
}
	...
	)
{
    ...
    if (bUSBBusIrqStat & INT_SDE)
    {
        ...
        {
            RETAILMSG(dbg,(_T("[UFNPDD] INT_SDE_EXEPTION Occured! \r\n")));
        }
    ...
    if (bUSBBusIrqStat & INT_OTG)
    {
        ...
        {	
            RETAILMSG(dbg,(_T("[UFNPDD] OTG Cable Detached\r\n")));
            pContext->pfnNotify(pContext->pvMddContext, UFN_MSG_BUS_EVENTS, UFN_DETACH);
            pContext->attachedState = UFN_DETACH;
	...
	)
{
	...
	{			
		if (dwDaint & EP0_IN_INT)
		{
			...
			{
				RETAILMSG(dbg,(_T("[UFNPDD] Time Out EP0\r\n")));
		...
		for(DWORD dwEndpoint = 1; dwEndpoint < ENDPOINT_COUNT; ++ dwEndpoint)
		{
			...
			if ((dwDaint & dwEpBit) && (dwDaintMsk & (0x1<<dwEndpoint)))
			{
				...
				{
					RETAILMSG(dbg,(_T("[UFNPDD] Time Out EP%d\r\n"),dwEndpoint));
					RETAILMSG(dbg,(_T("[UFNPDD] Transmit Ready Count : %d\r\n"), pContext->dwXmitReadyCnt));
					RETAILMSG(dbg,(_T("[UFNPDD] Transmitting EP 0x%x : %d\r\n"), dwEndpoint, pContext->dwXmittingEP));
					for(DWORD dwEndpoint = 0; dwEndpoint < ENDPOINT_COUNT; ++dwEndpoint)
					{
						if(pContext->dwInEPRunning[dwEndpoint])
						{
							RETAILMSG(dbg,(_T("[UFNPDD]EP%d is running\r\n"),dwEndpoint));
						}
					}
	...
	)
{
    ...
    {
        dwRet = GetLastError();
        RETAILMSG(dbg, (_T("%s MmMapIoSpace: FAILED\r\n"), pszFname));
        goto CleanUp;
    }
    ...
    )
{
	...
	RETAILMSG(dbg,(TEXT("[UFNPDD] USB IST START\r\n")));
    ...
    while (!pContext->fExitIST)
    {
        ...
        while (TRUE)
        {
	     ...
	     RETAILMSG(dbg,(TEXT("OTG Int \r\n")));
            ...
            {
            			RETAILMSG(dbg,(TEXT("Handle OTG Int Begin \r\n")));
				HandleUSBEvent(pContext);
				InterruptDone(pContext->dwSysIntr);
				RETAILMSG(dbg,(TEXT("Handle OTG Int End \r\n")));
            }
            else
            {
	...
	else //USB_OUT_TRANSFER
	{
		...
		else 
		{
			...
			if( pContext->dwUSBClassInfo != USB_MSF)
			{
				...
				{
					RETAILMSG(dbg,(TEXT("Ring buffer is buffering... Late processing!!!\r\n")));
    ...
    )
{
    if (cpsNew == D0)
    {
        RETAILMSG(dbg,(_T("[UFNPDD] USB_POWER : D0\r\n")));

		InterruptDone(pContext->dwSysIntr);

		if (pContext->FirstCall == FALSE)
		{
    ...
    {
        RETAILMSG(dbg,(_T("[UFNPDD] USB_POWER : D4\r\n")));
        SetSoftDisconnect();        
    ...
    )
{
	...
	RETAILMSG(dbg,(_T("[UFNPDD] USB DRIVER VERSION : %d\r\n"), USB_DD_VERSION));
	...
	if (USBCurrentDriver((PUFN_CLIENT_INFO)&currentDriver)) 
	{
		if(!wcscmp(currentDriver.szName, TEXT("RNDIS")) ||!wcscmp(currentDriver.szName, TEXT("rndis")))
		{
            ...
            RETAILMSG(dbg,(_T("[UFNPDD] USB RNDIS Function Class Enabled\r\n")));
		}
		else if(!wcscmp(currentDriver.szName, TEXT("Serial_Class")) ||!wcscmp(currentDriver.szName, TEXT("serial_class")))
		{
            ...
            RETAILMSG(dbg,(_T("[UFNPDD] USB Serial Function Class Enabled\r\n")));
		}
		else if(!wcscmp(currentDriver.szName, TEXT("Mass_Storage_Class")) ||!wcscmp(currentDriver.szName, TEXT("mass_storage_class")))
		{
			...
			pContext->dwUsingEPCnt = 3;
            RETAILMSG(dbg,(_T("[UFNPDD] USB Mass Storage Function Class Enabled\r\n")));
		}
	} 
	else 
	{
        RETAILMSG(dbg,(_T("[UFNPDD] USB Function Class Read Error!!!!!!!\r\n")));
	}
    ...
    )
{
	...
	if(bAddress > 127)
	{
	    RETAILMSG(dbg,(_T("[UFNPDD] Invalide Address - bAddr:%x\r\n"),bAddress));
	...
	)
{
    ...
    else
    {
        ...
        {
            RETAILMSG(dbg, (_T("[UFNPDD] dii.dwIrq = %d\n\r"), dii.dwIrq));
	...
	pContext->hBootCompleteEvent = OpenEvent(EVENT_ALL_ACCESS, FALSE, TEXT("OEM/BootCompleteEvent"));
	if (pContext->hBootCompleteEvent == NULL)
	{
		RETAILMSG(dbg, (TEXT("[UFNPDD] ERROR: Notify OpenEvent() FAIL 0x%08X\r\n"), GetLastError()));
		CloseHandle(pContext->hBootCompleteEvent);
		pContext->hBootCompleteEvent = NULL;
	}
