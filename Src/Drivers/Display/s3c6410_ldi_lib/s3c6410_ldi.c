...
#define SPI_DELAY        100//100        // for 667MHz    // 50
...
//        LCD_SetALC();
    ...
    case LDI_QC430QV_RGB:
        ...
        break;
    case LDI_ZQ65_RGB:
        LDI_INF((_T("[LDI:INF] LDI_initialize_LCD_module() : Type [%d] LDI_ZQ65RGB_RGB\n\r"), g_ModuleType));
        LDI_ZQ65RGB_initialize();
        LDI_ZQ65RGB_power_on();
        break;
...
LDI_ERROR LDI_deinitialize_LCD_module(void)
{
    ...
    case LDI_SMRP_LTE480WV_RGB:
        ...
        break;
    case LDI_ZQ65_RGB:
	 LDI_ZQ65RGB_power_off();
        LDI_INF((_T("[LDI:INF] LDI_deinitialize_LCD_module() : Type [%d] LDI_ZQ65_RGB\n\r"), g_ModuleType));
	 break;
    default:
        LDI_ERR((_T("[LDI:ERR] LDI_deinitialize_LCD_module() : Unknown LCD Module Type [%d]\n\r"), g_ModuleType));
        error = LDI_ERROR_ILLEGAL_PARAMETER;
        break;
    }
...
LDI_ERROR LDI_fill_output_device_information(void *pDevInfo)
{
    ...
    case LDI_LTV350QV_RGB:
        ...
        pDeviceInfo->uiWidth = 240;//320;
        pDeviceInfo->uiHeight = 320;//240;
        pDeviceInfo->VBPD_Value = 1;//5;
        pDeviceInfo->VFPD_Value = 11;//3;
        pDeviceInfo->VSPW_Value = 20;//4;
        pDeviceInfo->HBPD_Value = 8;//5;
        pDeviceInfo->HFPD_Value = 16;//3;
        pDeviceInfo->HSPW_Value = 29;//10;
...
case LDI_ZQ65_RGB:
	pDeviceInfo->RGBOutMode = DISP_16BIT_RGB565_P;
        ...
        pDeviceInfo->VFPD_Value =10;
        pDeviceInfo->VSPW_Value =3;
        pDeviceInfo->HBPD_Value = 114;
        pDeviceInfo->HFPD_Value = 16;
        ...
        pDeviceInfo->VCLK_Direction = CLKDIR_DIVIDED;
        pDeviceInfo->Frame_Rate = 60;
        RETAILMSG(1,(TEXT("Config zq65rgb:RGBOutMode:%d\n\r"),DISP_16BIT_RGB565_P));
break; 
    default:
...
static void LDI_LTS222QV_spi_port_disable(void)
{
    ...
    g_pGPIOReg->SPCON |= 0x1;        // RGB I/F
    g_pGPIOReg->GPCDAT |= ((1<<5)|(1<<6)|(1<<7));        // Clk, MOSI -> High
...
//denis_wei add to turn on the backlight  2009-09-28
	g_pGPIOReg->GPFCON &= ~(0x03<<28);		// Set GPF14 to OUTPUT mode.
	g_pGPIOReg->GPFCON |= (0x1 << 28);
	g_pGPIOReg->GPFDAT  |= (1<<14);
	//add end 2009-09-28
...
{
//GPI 8 ->DISPLAY GPI 9 ->POWER ON CLEAR GPI0 ->UD(LOW FOR NORMAL)    GPI1 ->RL(HIGH FOR NORMAL)
g_pGPIOReg->GPICON = (g_pGPIOReg->GPICON & ~((3<<16)|(3<<18)|(3<<0)|(3<<2))) | ((1<<16)|(1<<18)|(1<<0)|(1<<2));
g_pGPIOReg->GPIDAT |= (1<<8)|(1<<1)|(1<<9);
g_pGPIOReg->GPIDAT &= ~(1<<0);

g_pGPIOReg->GPFDAT &= ~(1<<15);
g_pGPIOReg->GPFCON = (g_pGPIOReg->GPFCON & ~(3<<30)) | (1<<30); // LCD EN
...
static void LDI_ZQ65RGB_power_on(void)
{
	...
	g_pGPIOReg->GPLCON0 = (g_pGPIOReg->GPLCON0 & ~(0xF<<28) ) | (1<<28);
	g_pGPIOReg->GPLDAT |= (1<<7);    //LCD POWER
	//RETAILMSG(1,(TEXT("LCD power on!!!!\r\n")));
	delay1();
	delay1();
	delay1();
...
//RETAILMSG(1,(TEXT("LCD power off!!!!\r\n")));
} 
