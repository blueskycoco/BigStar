...
BOOL DMA_request_channel(DMA_CH_CONTEXT *pCtxt, DMA_SOURCE DMASrc)
{
	...
	case SDMA_SPI0_TX:
	case SDMA_SPI0_RX:
	case SDMA_I2S_V40_TX:
	case SDMA_I2S_V40_RX:
		pCtxt->Controller = SDMAC0;
		break;  
	case SDMA_PCM1_TX:
	case SDMA_PCM1_RX:
	case SDMA_I2S1_TX:
	...
	case SDMA_SPI1_TX:
	case SDMA_SPI1_RX:
	case SDMA_AC97_PCMOUT:
	case SDMA_AC97_PCMIN:
	case SDMA_AC97_MICIN:
	case SDMA_PWM:
	case SDMA_IRDA:
	case SDMA_EXTERNAL:
	case SDMA_SECU_RX:
	case SDMA_SECU_TX:
		pCtxt->Controller = SDMAC1;
		break;
	...
	if (pCtxt->Controller == SDMAC0)	// Find available channel in SDMAC0
	{
		for (i=0; i<8; i++)
		{
			if (i == 3) continue;	// for OneNAND SDMA channel
		...
		if (i < 8)		// there is available channel
		{
			DMA_INF((_T("[DMA:INF] DMA_request_channel() : Ch[%d] in SDMAC0 is Available for DMASrc[%d]\n\r"), i, DMASrc));
			pCtxt->Channel = (DMAC_CH)i;
			pCtxt->dwIRQ = IRQ_SDMA0_CH0+i;
			pCtxt->pCHReg = (void *)pDMACHReg;
			pCtxt->bValid = TRUE;
			DMA_dmac0_enable();
		}
		else if (DMASrc == DMA_MEM)	// SDMA MEM2MEM can use channel in SDMAC1
		{
			pCtxt->Controller = SDMAC1;
		}
		else			// there is no available channel
		{
			DMA_ERR((_T("[DMA:ERR] DMA_request_channel() : No Available Channel in SDMAC0 for DMASrc[%d]\n\r"), DMASrc));
			bRet = FALSE;
			goto CleanUp;
		}
	}

	if (pCtxt->Controller == SDMAC1)	// Find available channel in SDMAC1
	{
		for (i=0; i<8; i++)
		{
			if (i == 3) continue;	// for OneNAND SDMA channel
		...
		if (i < 8)		// there is available channel
		{
			DMA_INF((_T("[DMA:INF] DMA_request_channel() : Ch[%d] in SDMAC1 is Available for DMASrc[%d]\n\r"), i, DMASrc));
			pCtxt->Channel = (DMAC_CH)i;
			pCtxt->dwIRQ = IRQ_SDMA1_CH0+i;
			pCtxt->pCHReg = (void *)pDMACHReg;
			pCtxt->bValid = TRUE;
			DMA_dmac1_enable();
		}
		...
		{
			DMA_ERR((_T("[DMA:ERR] DMA_request_channel() : No Available Channel in SDMAC1 for DMASrc[%d]\n\r"), DMASrc));
			bRet = FALSE;
			goto CleanUp;
		}
	}
...
BOOL DMA_release_channel(DMA_CH_CONTEXT *pCtxt)
{
	...
	if ((pCtxt->Controller == DMAC0) || (pCtxt->Controller == SDMAC0))		// Release channel in DMAC0, SDMAC0
	{
		DMA_dmac0_disable();
	}
	else if ((pCtxt->Controller == DMAC1) || (pCtxt->Controller == SDMAC1))	// Release channel in DMAC1, SDMAC1
	{
		DMA_dmac1_disable();
	}
...
DMA_ERROR DMA_initialize_channel(DMA_CH_CONTEXT *pCtxt, BOOL bSync)
{
	...
	//-------------------
	// SDMA Source in SDMC0
	//-------------------
	case SDMA_UART0_TX:
		pCtxt->SrcAHBM = AHB_M1;			// Memory
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_UART0_TX;		// UART0_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_UART0_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_UART0_RX;		// UART0_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_UART1_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_UART1_TX;		// UART1_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_UART1_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_UART1_RX;		// UART1_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_UART2_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_UART2_TX;		// UART2_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_UART2_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_UART2_RX;		// UART2_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_UART3_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_UART3_TX;		// UART3_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_UART3_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_UART3_RX;		// UART3_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_PCM0_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_PCM0_TX;		// PCM0_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_PCM0_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_PCM0_RX;		// PCM0_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_I2S0_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_I2S0_TX;		// I2S0_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_I2S0_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_I2S0_RX;		// I2S0_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_SPI0_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_SPI0_TX;		// SPI0_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_SPI0_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_SPI0_RX;		// SPI0_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	...
	case SDMA_I2S_V40_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC0_I2S_V40_TX;	// I2S_V40_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_I2S_V40_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC0_I2S_V40_RX;	// I2S_V40_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;

	//-------------------
	// SDMA Source in SDMC1
	//-------------------
	case SDMA_PCM1_TX:
		pCtxt->SrcAHBM = AHB_M1;			// Memory
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC1_PCM1_TX;		// PCM1_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_PCM1_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC1_PCM1_RX;		// PCM1_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_I2S1_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC1_I2S1_TX;		// I2S1_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_I2S1_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC1_I2S1_RX;		// I2S1_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_SPI1_TX:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC1_SPI1_TX;		// SPI1_TX
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_SPI1_RX:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC1_SPI1_RX;		// SPI1_RX
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_AC97_PCMOUT:
		...
		pCtxt->DstAHBM = AHB_M2;			// Peripheral
		pCtxt->SrcPeri = 0;					// Memory (Don't Care)
		pCtxt->DstPeri = SDMAC1_AC97_PCMOUT;	// AC97_PCMOUT
		pCtxt->FlowCtrl = MEM_TO_PERI;		// Memory -> Peripheral
		break;
	case SDMA_AC97_PCMIN:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC1_AC97_PCMIN;	// AC97_PCMIN
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_AC97_MICIN:
		pCtxt->SrcAHBM = AHB_M2;			// Peripheral
		pCtxt->DstAHBM = AHB_M1;			// Memory
		pCtxt->SrcPeri = SDMAC1_AC97_MICIN;	// AC97_MICIN
		pCtxt->DstPeri = 0;					// Memory (Don't Care)
		pCtxt->FlowCtrl = PERI_TO_MEM;		// Peripheral -> Memory
		break;
	case SDMA_PWM:
...
DMA_INT_STATUS DMA_get_interrupt_status(DMA_CH_CONTEXT *pCtxt)
{
	...
	if ((pCtxt->Controller == DMAC0) || (pCtxt->Controller == SDMAC0))		// channel in DMAC0, SDMAC0
	{
	...
	else if ((pCtxt->Controller == DMAC1) || (pCtxt->Controller == SDMAC1))	// channel in DMAC1, SDMAC1
	{
