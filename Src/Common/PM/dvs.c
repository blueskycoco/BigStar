...
static DWORD g_dwCurrentIdleTime, g_dwIdleRate;

static BOOL g_bUpdatedDVSFine = FALSE;
static BOOL g_bUpdatedDVSCoarse = FALSE;
...
void UpdateDVS(void)
{
    ...
    #if (DVFS_TEST == TRUE)
    static DWORD TempLevel = 0;
    static DWORD TempMode = 0;
    #endif
    ...
    if ( (dwCurrentMSec - tLastLogFine.dwTickCount) >=  DVS_UPDATE_PERIOD_FINE )
    {
        ...
        tCurLog.dwIdleRate = g_dwIdleRate = (100*(dwCurrentIdleMSec-tLastLogFine.dwIdleCount))/(dwCurrentMSec-tLastLogFine.dwTickCount);
        ...
        if (tCurLog.dwIdleRate < ShiftUpRate)
        {
#ifdef DVS_LEVEL_PROFILE
            if (g_bProfileDVS)
            {
...
#endif
            if (g_bUpdatedDVSFine == TRUE)  // Perform DVFS if UpdateDVS() is called without calling PerformDVS() from OEMIdle.
            {
                PerformDVS();
            }
            else
            {
                g_bUpdatedDVSFine = TRUE;
            }
        }
        else
        {
            ...
            {
                //OALMSG(TRUE, ((L"DFS[%d:%d]\r\n"), g_CurrentLevel, g_dwIdleRate));
                tCurLog.dwIdleRate = g_dwIdleRate = (100*(dwCurrentIdleMSec-tLastLogCoarse.dwIdleCount))/(dwCurrentMSec-tLastLogCoarse.dwTickCount);
                ...
                if (g_bUpdatedDVSCoarse == TRUE)  // Perform DVFS if UpdateDVS() is called without calling PerformDVS() from OEMIdle.
                {
                    PerformDVS();
                }
                else
                {
                    g_bUpdatedDVSCoarse = TRUE;
                }
            }
        }

        #if (DVFS_TEST == TRUE)
        ChangeDVSLevel(TempLevel);

        if (TempMode == 0)  // incre
        {
            if (TempLevel < 3) TempLevel++;
            else if (TempLevel == 3) TempMode = 1;
        }
        else                // decre
        {
            if (TempLevel > 0) TempLevel--;
            else if (TempLevel == 0) TempMode = 0;
        }
        #endif
    }
}
...
void PerformDVS(void)
{
    ...
    if ((g_bUpdatedDVSFine == TRUE) && (g_dwIdleRate < ShiftUpRate))    // Pump Up the Clock
    {
        #if (DVFS_TEST != TRUE)
    ...
    else if ((g_bUpdatedDVSCoarse == TRUE) && (g_dwIdleRate > ShiftDownRate))    // Pump Down the Clock
    {
        #if (DVFS_TEST != TRUE)
    ...
    g_bUpdatedDVSFine = FALSE;
    g_bUpdatedDVSCoarse = FALSE;
}
...
__inline void ChangeClockDivider(SYSTEM_ACTIVE_LEVEL NewLevel)
{
    DWORD dwNewCLKDIV = 0;
